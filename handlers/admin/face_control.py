...
from database.req.face_control import add_face_control, get_face_control, list_face_control, remove_face_control
from handlers.error import safe_send_message
from handlers.states import FaceControlState
from keyboards.keyboards import face_control_menu_kb, back_to_face_control, face_controls_list, yes_no_face
...
async def face_control_add(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "ID можно получить, если пользователь перешлет сообщение от @getmyid_bot", reply_markup=back_to_face_control()
    )
