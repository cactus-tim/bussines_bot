"""
Giveaway Result Retrieval
Handles additional giveaway winner selection for club_events.
"""
...
from database.req import (
    get_user, get_all_events, get_all_hosts_in_event_orgs,
    get_host_by_org_name, get_add_winner
)
...
async def get_result2(message: Message, state: FSMContext):
    ...
    await safe_send_message(
        bot,
        message,
        f'@{winner.handler} - победитель розыгрыша от @{host.org_name}'
    )
